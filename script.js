      ...
      if (style === '90s') {
        ...
        if (lastPoint) {
          ...
          if (dist < 20) {
            ...
            // Add slight random offset for "sketch" feel
            ctx.lineTo(p.x + (Math.random() - 0.5) * 2, p.y + (Math.random() - 0.5) * 2);
            // Multiple strokes for pencil shading effect
            ctx.stroke();
            ctx.beginPath();
            ctx.moveTo(lastPoint.x + (Math.random() - 0.5), lastPoint.y + (Math.random() - 0.5));
            ctx.lineTo(p.x, p.y);
            ctx.stroke();
          }
        }
